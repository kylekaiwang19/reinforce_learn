...
class agent:
    ...
    def optim_policy(self, obs, act, rew, value, logp_old):
        # implements one single iteration in ppo with ac style
        self.policy_optim.zero_grad()
        logp = self.get_logp(obs, act)
        adv = rew - value
        r = torch.exp(logp - logp_old)
        loss = -torch.minimum(r * adv, torch.clamp(r, 1-self.clip_r, 1+self.clip_r) * adv).mean()
        loss.backward()
    ...
    def train_eps(self):
        ...
        while not done:
            ...
            logp_old = self.get_logp(torch.as_tensor(obs, dtype=torch.float32), 
                                    torch.as_tensor(act, dtype=torch.float32))
            eps_logp.append(logp_old)
            ...
            if not done:
                eps_obs.append(obs)
            eps_len += 1
        # for every episode, optimize the policy and value function multiple times
        for i in range(self.policy_iters):
            self.optim_policy(torch.as_tensor(eps_obs, dtype=torch.float32), 
                            ...
                            torch.as_tensor(eps_logp, dtype=torch.float32))
    ...
    def train(self):
        for i in range(self.num_eps):
            ...
            print(f'episode {i+1}, eposide length={eps_len}')
